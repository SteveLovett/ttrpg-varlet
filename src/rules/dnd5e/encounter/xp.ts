/** XP value for a single monster by CR (DMG 5e table, common values). */
...
/** Per-character XP budget thresholds by party level (DMG). */
...
export function partyXpBudget(
  ...
  const perPc = THRESHOLDS[lvl]?.[difficulty] ?? THRESHOLDS[5][difficulty]
  return perPc * size
}
